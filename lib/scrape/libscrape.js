...
async function blackboxV4(query = '', allMessages = [{ user: query }], stream = false, clickedContinue = false) {
    try {
...
async function niji(prompt, aspectRatio, imageUrl = null) {
  try {
    const res = await axios.post(`https://project-niji.onrender.com/api/generate?apikey=rehat&key=siam&prompt=${prompt}&aspectRatio=${aspectRatio}`);
    return res.data;
  } catch (error) {
